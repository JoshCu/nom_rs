//! `WaterMain`, replayed against the Fortran's own recording.
//!
//! Same method as `energy_vs_fortran.rs`: the complete recorded pre-state is loaded, the Rust
//! runs, and the complete recorded post-state must match bit for bit.
...
#[test]
fn water_main_reproduces_the_fortran() {
    ...
    assert!(!pairs.is_empty(), "no WaterMain records");
