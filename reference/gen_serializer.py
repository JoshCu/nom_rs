...
#
# `paramstate` is not a derived type of its own. `parameters_type` is mostly what its name says
# -- read once from the tables and never touched again -- but the physics writes a handful of
# its components every timestep: PHENOLOGY recomputes LAI, SAI, ELAI, ESAI and FVEG from the
# day of year and the snow depth, and EnergyModule sets VAI and VEG. Those are per-timestep
# state wearing a parameter's name, and a fixture that recorded them once would be unable to
# check any of it. So they are dumped per record as well, as a subset of the same type.
PER_RECORD = ["domain", "forcing", "energy", "water", "paramstate"]
STATIC = ["levels", "options", "parameters"]
# The Fortran derived type each of the above is drawn from.
SOURCE_TYPE = {t: t for t in PER_RECORD + STATIC}
SOURCE_TYPE["paramstate"] = "parameters"
...
# Files that define or fill `parameters_type` rather than using it. Anything else that assigns
# to a component is the physics, and that component is per-timestep state.
NOT_PHYSICS = ("ParametersType.f90", "ParametersRead.f90", "DiffTestModule.f90")

ASSIGN = re.compile(r"parameters%(\w+)\s*=(?!=)")


def mutable_parameters(src):
    """Components of `parameters_type` that the physics assigns to.

    Derived rather than listed, because a list would rot: upstream adding one more
    `parameters%X = ...` to a physics module would otherwise leave X recorded once and checked
    never, and the port could drift on it indefinitely without a test noticing.
    """
    names = set()
    for path in sorted(src.glob("*.f90")):
        if path.name in NOT_PHYSICS or path.name.endswith("_driver.f90"):
            continue
        for line in path.read_text(errors="replace").splitlines():
            code = line.split("!")[0]
            names.update(m.lower() for m in ASSIGN.findall(code))
    if not names:
        raise SystemExit(f"{src}: found no physics assignments to parameters% -- check ASSIGN")
    return names
...
def emit_fortran(schema):
    use = "\n".join(
        f"  use {t.capitalize()}Type" if t != "levels" else "  use LevelsType"
        for t in dict.fromkeys(SOURCE_TYPE[t] for t in TYPES)
    )
    dumps, calls = [], []
    for t in TYPES:
        var = SOURCE_TYPE[t]
        body = "\n".join(fortran_write(var, f) for f in schema[t])
        dumps.append(
            f"  subroutine dt_dump_{t}({var})\n"
            f"    type({var}_type), intent(in) :: {var}\n"
            f"{body}\n"
            f"  end subroutine dt_dump_{t}\n"
        )
    per = "\n".join(f"    call dt_dump_{t}({SOURCE_TYPE[t]})" for t in PER_RECORD)
    per_args = ", ".join(dict.fromkeys(SOURCE_TYPE[t] for t in PER_RECORD))
    per_decl = "\n".join(
        f"    type({t}_type), intent(in) :: {t}"
        for t in dict.fromkeys(SOURCE_TYPE[t] for t in PER_RECORD)
    )
    static_args = ", ".join(STATIC)
    static_decl = "\n".join(f"    type({t}_type), intent(in) :: {t}" for t in STATIC)
    static = "\n".join(f"    call dt_dump_{t}({SOURCE_TYPE[t]})" for t in STATIC)
...
def main():
    ...
    for t in TYPES:
        ...
        fields = parse_type(src / f"{source.capitalize()}Type.f90", f"{source}_type")
        schema[t] = [f for f in fields if (t, f["name"]) not in EXCLUDE]

    mutable = mutable_parameters(src)
    schema["paramstate"] = [f for f in schema["paramstate"] if f["name"].lower() in mutable]
    missing = mutable - {f["name"].lower() for f in schema["paramstate"]}
    if missing:
        raise SystemExit(f"physics assigns to parameters%{sorted(missing)}, not declared there")
    print(f"mutable parameters: {sorted(f['name'] for f in schema['paramstate'])}",
          file=sys.stderr)
